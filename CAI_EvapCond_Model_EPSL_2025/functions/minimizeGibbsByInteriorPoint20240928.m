...
n_species_0 = equilibrium_params.n_species;
formula_matrix = equilibrium_params.formula_matrix;
b_elements = equilibrium_params.b_elements;
tau_sequence = equilibrium_params.tau_sequence;
...
equilibrium_params.n_species = n_species;
...
for outer_iteration = 1:length(tau_sequence)
    ...
    while max( abs(delta_n_and_lambda(1:N)) ./ n_species ) > 1e-5 && ...
        ...
        equilibrium_params.n_species = n_species_prov;                                  % 2024/09/28
        ...
        while merit_prov > merit_ref  && ...
                abs((merit_prov - merit_ref)/merit_ref) > 1e14 % added latter part       Oops, this condition will never be met (1e14 is a typo for 1e-14), but solution seems to be reached without this check anyway
